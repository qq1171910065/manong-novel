...
<template>
  <div class="space-y-4 max-h-96 overflow-y-auto p-1">
    <div v-for="(chapter, index) in localOutline" :key="chapter.id || chapter.chapter_number || index" class="p-4 border border-gray-200 rounded-lg bg-gray-50">
      <div class="flex items-center mb-2 gap-2 flex-wrap">
        ...
        <input 
          ...
          class="flex-grow min-w-[8rem] p-1 border-b-2 border-gray-300 focus:border-indigo-500 outline-none transition"
          placeholder="章节标题"
        />
        <label class="flex items-center gap-1 text-xs text-gray-500 shrink-0">
          ...
          <input
            type="number"
            v-model.number="chapter.target_word_count"
            min="800"
            step="100"
            class="w-20 p-1 border border-gray-300 rounded text-sm text-gray-700"
            placeholder="3200"
          />
        </label>
...
import { ensureChapterOutline } from '@renderer/services/novel/blueprint-asset';
...
watch(() => props.modelValue, (newVal) => {
  syncing = true;
  localOutline.value = (newVal || []).map((item) => ensureChapterOutline(item));
  nextTick(() => {
    syncing = false;
  });
}, { immediate: true });
