...
<template>
  <div class="space-y-4 max-h-96 overflow-y-auto p-1">
    <div v-for="(faction, index) in localFactions" :key="faction.id || index" class="p-4 border border-gray-200 rounded-lg bg-gray-50 relative">
...
const addFaction = () => {
  localFactions.value.push({ id: randomUUID(), name: '', description: '' });
};
