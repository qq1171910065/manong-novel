...
<template>
  <div class="space-y-6">
    ...
    <!-- 状态提示 -->
    <div v-if="!selectedChapter?.content" class="md-card md-card-filled p-4" style="border-radius: var(--md-radius-lg); background-color: var(--md-primary-container);">
    ...
    <section class="novel-version-panel">
      ...
      <div class="novel-version-panel__actions">
        <p
          v-if="availableVersions.length < 2"
          class="md-body-small md-on-surface-variant mr-auto"
        >
          需至少 2 个版本才可评审，请重新生成章节
        </p>
        <button
          @click="$emit('evaluateChapter')"
        ...
        >
          ...
          <span v-else>
            {{ isConfirmedVersion(selectedVersionIndex) ? '当前版本' : '确认选择此版本' }}
          </span>
        </button>
      </div>
    </section>
