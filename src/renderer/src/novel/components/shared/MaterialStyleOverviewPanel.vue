...
const overviewItems = computed(() => [
  { key: 'genre', label: getMaterialFieldLabel('genre'), value: props.draft.genre },
  { key: 'style', label: getMaterialFieldLabel('style'), value: props.draft.style },
  { key: 'tone', label: getMaterialFieldLabel('tone'), value: props.draft.tone },
  { key: 'writingHints', label: getMaterialFieldLabel('writingHints'), value: props.draft.writingHints },
])
...
const pendingCount = computed(
  () => overviewItems.value.filter((item) => !item.value.trim()).length
)
...
<template>
  <aside class="concept-panel material-style-overview" aria-label="文风设定">
    <div class="concept-panel__head">
      <h3 class="concept-panel__title">文风设定</h3>
    ...
    <div v-if="coverUrl" class="material-style-overview__cover">
      <img :src="coverUrl" alt="文风配图" />
    </div>
      ...
      >
        ...
        <div v-show="checklistOpen" class="concept-panel__accordion-body">
          <ul class="concept-panel__list">
            <li
              v-for="item in [...cardItems, ...overviewItems]"
              :key="item.key"
...
<style scoped>
.material-style-overview__cover {
  overflow: hidden;
  border-radius: 12px;
...
.material-style-overview__cover img {
  display: block;
  width: 100%;
  height: 100%;
  object-fit: cover;
