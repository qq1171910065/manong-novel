...
<template>
  <div class="space-y-4 max-h-96 overflow-y-auto p-1">
    <div v-for="(location, index) in localLocations" :key="location.id || index" class="p-4 border border-gray-200 rounded-lg bg-gray-50 relative">
...
watch(() => props.modelValue, (newVal) => {
  syncing = true;
  localLocations.value = (newVal || []).map((item) => ensureWorldListItem(item));
  nextTick(() => {
    syncing = false;
  });
}, { immediate: true });
...
const addLocation = () => {
  localLocations.value.push({ id: randomUUID(), name: '', description: '' });
};
