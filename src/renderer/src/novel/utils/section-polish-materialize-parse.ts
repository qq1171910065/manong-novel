import { parseLlmJsonObject, sanitizeJsonLikeText, unwrapMarkdownJson } from '@renderer/services/novel/json-utils'
import {
...
import { extractCharactersFromPayload } from './section-polish-batch'
  ...
  if (parsed.blueprint_updates || parsed.section_update) {
    ...
    if (chars.length) {
      return {
        ...parsed,
        characters: chars
          .map((c) => normalizeMaterializeCharacter(c as Record<string, unknown>))
