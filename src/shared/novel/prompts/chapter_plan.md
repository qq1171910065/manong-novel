...
```json
{
  ...
  "opening_continuation": "本章开场 2-3 句的执行指令：如何从上一章结尾最后一拍自然续写（时间/地点/动作/情绪）",
  "forbidden": [
    "禁止跨章总结",
    "禁止主角知道未获得信息",
    "禁止突然提及未登场角色姓名",
...
### opening_continuation（开场衔接，必填）
- 用 2-3 句话描述**本章第一段**如何承接 `prior_ending` 的最后一拍。
- 必须明确：时间是否连续、地点是否同一、POV 角色当下状态、第一个动作/感官细节从哪接续。
- 禁止写「然后故事继续」这类空话；禁止无交代硬切到全新场景（除非大纲明确要求时间跳跃且需写出过渡句）。
