import type { Character, NovelProject } from '@shared/novel/types'
...
function extractMaterialSectionFromPrompt(initialPrompt: string): string | null {
  ...
  return initialPrompt.slice(idx).trim()
}
...
export function buildConceptMaterialContext(project: NovelProject): string {
  ...
  const fromPrompt = project.initial_prompt
    ? extractMaterialSectionFromPrompt(project.initial_prompt)
    : null
